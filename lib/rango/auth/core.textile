...
Rango::Auth is an authentication framework for use with the
Rango:: web framework.

Rango::Auth does not try to dictate what you should use as a user model, or how
...
objects in the session.  This is in fact the guiding principle of Rango::Auth.  The Session is
...
Rango::Auth makes use of Rango::'s exception handling facilities which return correct
...
It is possible to use Rango::Auth with any object as a _user_ object, provided
...
This is very similar to the BootLoader process in Rango::s initialization.
You declare a class that inherits from Rango::Authentication::Strategy and
define an instance method @run!@

<pre><code>
  class PasswordStrategy &lt Rango::Authentication::Strategy
...
<pre><code>
    class PasswordLoginBasicAuth &lt Rango::Authentication::Strategy
      def run!
...
By default the strategy#user_class method will defer to Rango::Authentication#user_class.  You can
set which is the "default class" that Rango::Authentication will use in the provided strategies by
setting it in Rango::.root/merb/merb-auth/setup.rb

<pre><code>

    Rango::Authentication.user_class = Person
...
There is no default class set for Rango::Authentication.user_class by default
...
<pre><code>
  class AbstractStrategy < Rango::Authentication::Strategy
    abstract!
  end
</code></pre>
...
<pre><code>
  Rango::Authentication.activate!(:defualt_password_form)
</code></pre>

You can easily mix this in with your own strategies. In you Rango::.root/merb-auth/strategies.rb
<pre><code>
  ...
  Rango::Authentication.activate!(:default_openid)

  class AnotherStrategy < Rango::Authentication::Strategy
    def run!
      #...
    end
  end
...
This will collect them in order of declaration.  i.e.:
  MyStrategy, Rango::Authentication::Strategies::Basic::OpenID, AnotherStrategy
...
@Rango::Authentication.default_strategy_order@ will return an array of
...
With Rango::Auth you can protect routes rather than individual actions on a controller.
...
<pre><code>
  class ApiMethods < Application
    before :ensure_authenticated, :with => [
                                            Rango::Authenticated::Strategies::Basic::Form,
                                            Rango::Authenticated::Strategies::Basic::BasicAuth,
                                            Rango::Authenticated::Strategies::Basic::OpenID,
                                           ],
                                  :only => [:index]
    ...
    private
    def machine_only
      ensure_authentiated Rango::Authenticated::Strategies::Basic::OAuth, Rango::Authenticated::Strategies::Basic::BasicAuth
    end
  end
</code></pre>
...
You need to tell Rango::Auth how to serialize your object into
...
* Overwrite Rango::Authentication#error_message  The return of this method is
...
* Provide a default message when you declare your before filter.
  ...
  When you pass a message, it will replace the Rango::Authentication#error_message default for this
  action
...
Rango::Authentication.after_authentication which is designed for this.
