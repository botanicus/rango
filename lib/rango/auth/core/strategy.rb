class Rango
  class Authentication
    ...
    #
    # @params [Rango::Authentiation::Strategy,Rango::Authentication::Strategy]
    #
    # @public
    def self.default_strategy_order=(*order)
      order = order.flatten
      bad = order.select{|s| !s.ancestors.include?(Strategy)}
      raise ArgumentError, "#{bad.join(",")} do not inherit from Rango::Authentication::Strategy" unless bad.empty?
    ...
    # it simply makes it available through the Rango::Authentication.activate method
    #
    ...
    # The Rango::Authentication::Strategy is where all the action happens in the merb-auth framework.
    ...
    class Strategy
      ...
      class << self
        def inherited(klass)
          Rango::Authentication.strategies << klass
          Rango::Authentication.default_strategy_order << klass
        end

        # Use this to declare the strategy should run before another strategy
        def before(strategy)
          order =  Rango::Authentication.default_strategy_order
          order.delete(self)
      ...
      #
      # By default, Rango::Authentication.user_class is used.  This method allows for
      # particular strategies to deal with a different type of user class.
      #
      # For example.  If Rango::Authentication.user_class is Customer
      ...
      # @api overwritable
      def user_class
        Rango::Authentication.user_class
      end
