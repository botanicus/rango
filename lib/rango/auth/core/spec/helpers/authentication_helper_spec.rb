...
describe "Rango::AuthenticationHelper" do

  class ControllerMock < Rango::Controller
  ...
  before(:each) do
    ...
    class Kone < Rango::Authentication::Strategy
      def run!
  ...
  it "should not raise and Unauthenticated error" do
    ...
    end.should_not raise_error(Rango::Controller::Unauthenticated)
  end

  it "should raise an Unauthenticated error" do
    @controller = ControllerMock.new(Rango::Request.new({}))
    lambda do
      @controller.send(:ensure_authenticated)
  ...
  it "should accept and execute the provided strategies" do
    ...
    # before :ensure_authenticated, :with => [Authenticaiton::OAuth, Rango::Authentication::BasicAuth]
    controller = ControllerMock.new(fake_request)
