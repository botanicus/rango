...
describe "Rango::Authentication Session" do

  before(:each) do
    @session_class = Rango::CookieSession
  ...
  describe "Rango::Authentication" do
    it "Should be hookable" do
      Rango::Authentication.should include(Extlib::Hook)
    end
  ...
  describe "#authenticate" do
    ...
    before(:each) do
      class Sone < Rango::Authentication::Strategy
        def run!
          Viking.capture(Sone)
          params[:pass_1]
        end
      end
      class Stwo < Rango::Authentication::Strategy
        def run!
          Viking.capture(Stwo)
          params[:pass_2]
        end
      end
      class Sthree < Rango::Authentication::Strategy
        def run!
          Viking.capture(Sthree)
          params[:pass_3]
        end
      end
      class Sfour < Rango::Authentication::Strategy
        abstract!
    ...
    it "should use the Authentiation#error_message as the error message" do
      ...
      lambda do
        @auth.authenticate!(@request, @request.params)
      end.should raise_error(Rango::Controller::Unauthenticated, "BAD BAD BAD")
    end
    ...
    describe "Strategy loading as strings" do
      ...
      before :each do
        Rango::Authentication.reset_strategy_lookup!

        class Rango::Authentication::Strategies::Zone < Rango::Authentication::Strategy
          def run!
            Viking.capture(Rango::Authentication::Strategies::Zone)
            params[:z_one]
          end
        end
      end
      ...
      it "should allow a mix of strategies as strings or classes" do
        ...
        Viking.captures.should == %w(Rango::Authentication::Strategies::Zone Sone Stwo)
      end
    end
  ...
  describe "user_class" do
    it "should have User as the default user class if requested" do
      Rango::Authentication.user_class.should == User
    end
  end
  ...
  describe "redirection" do
    ...
    before(:each) do
      ...
      class FailStrategy < Rango::Authentication::Strategy
        def run!
      ...
      @a = Rango::Authentication.new(@request.session)
    end
    ...
    it "should stop processing the strategies if one redirects" do
      ...
      end.should_not raise_error(Rango::Controller::NotFound)
      @a.should be_halted
