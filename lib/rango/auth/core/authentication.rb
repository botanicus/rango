class Rango
  class Authentication
    ...
    # merb-auth authentication framework.  Rango::Authentication.user_class can
    ...
    def authenticate!(request, params, *rest)
      ...
      strategies = if rest.empty?
        if request.session[:authentication_strategies]
          ...
          Rango::Authentication.default_strategy_order
        end
      else
        request.session[:authentication_strategies] ||= []
      ...
      strategies.detect do |s|
        s = Rango::Authentication.lookup_strategy[s] # Get the strategy from string or class
        unless s.abstract?
      ...
      raise Rango::Controller::Unauthenticated, msg unless user
    ...
    #
    # This message is the default message passed to the Rango::Controller::Unauthenticated exception
    ...
    # When loading from string, strategies are loaded withing the Rango::Authentication::Strategies namespace
    ...
    def self.reset_strategy_lookup!
      @strategy_lookup = Mash.new do |h,k|
        ...
        when Class
          h[k] = k
        when String, Symbol
          h[k] = Rango::Authentication::Strategies.full_const_get(k.to_s)
        end
      end
    end
    ...
    def run_after_authentication_callbacks(user, request, params)
      Rango::Authentication.after_callbacks.each do |cb|
        user = case cb
        when Proc
