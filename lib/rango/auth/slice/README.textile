Rango::AuthSlicePassword ==================

A slice for the Rango:: framework that uses the merb-auth-core authentication
...
Rango::AutheSliceDefault[:no_default_strategies] = true
...
3. The default layout, as specified in Rango::Slices::config[:mauth_password_slice][:layout]
...
To see all available tasks for Rango::AuthSlicePassword run:
...
Rango::Plugins.config[:merb_slices] = { :queue => ["Rango::AuthSlicePassword", ...] }

# Setup your application to use Rango::AuthSlicePassword in an after_app_loads block

# To change the login or password field to a field of your choice
  Set
  Rango::Plugin.config[:"merb-auth"][:login_param]
  Rango::Plugin.config[:"merb-auth"][:password_param]
...
Rango::AutheSlicePassword[:no_default_strategies] = true
...
slice(:Rango::AuthSlicePassword, :name_prefix => nil, :path_prefix => "auth", :default_routes => false )
...
You'll need to setup the Rango::Authentication::Manager#fetch_user and store_user
...
Rango::-Slices is a Rango:: plugin for using and creating application 'slices' which
help you modularize your application. Usually these are reuseable extractions
from your main app. In effect, a Slice is just like a regular Rango:: MVC
application, both in functionality as well as in structure.

When you generate a Slice stub structure, a module is setup to serve as a
namespace for your controller, models, helpers etc. This ensures maximum
encapsulation. You could say a Slice is a mixture between a Rango:: plugin (a
Gem) and a Rango:: application, reaping the benefits of both.
...
this method is available to any kind of controller, so it can be used for Rango::
