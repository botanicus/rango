...
describe "redirect_back" do
  ...
  before(:all) do
    Rango::Config[:exception_details] = true
    clear_strategies!
    Rango::Router.reset!
    ...
    # class MyStrategy < Rango::Authentication::Strategy; def run!; request.env["USER"]; end; end
    ...
    class Application < Rango::Controller; end

    class Exceptions < Rango::Controller
      include Rango::Authentication::Mixins::RedirectBack

      def unauthenticated; end

    end

    class Sessions < Rango::Controller
  ...
  it "should set the return_to in the session when sent to the exceptions controller from a failed login" do
    r = request("/go_back")
    r.status.should == Rango::Controller::Unauthenticated.status
    r2 = login
