...
class Rango::Authentication
  module Strategies
    module Basic
      class OpenID < Rango::Authentication::Strategy
        def run!
        ...
        def openid_callback_url
          "#{request.protocol}://#{request.host}#{Rango::Router.url(:openid)}"
        end
        ...
        def on_success!(response, sreg_response)
          if user = find_user_by_identity_url(response.identity_url)
            ...
            redirect!(Rango::Router.url(:signup))
          end
        end
        ...
        # @api overwritable
        def openid_store
          ::OpenID::Store::Filesystem.new("#{Rango.root}/tmp/openid")
        end
